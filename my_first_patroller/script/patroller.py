...
import roslib; roslib.load_manifest('my_first_patroller'); roslib.load_manifest('ros_mary_tts');
import rospy
import actionlib
import tf
import time
...
class PatrollerServer:
	...
	def __init__(self):
		self.server = actionlib.SimpleActionServer('my_first_patroller', patrolAction, self.execute, False)
		self.server.start()
